# 2d-kinematics-review-findings Specification

## Purpose
定义从 `swim-side-kinematics.v1` 的 `AnnotationMetric` 生成有限、可解释的待复核二维运动学发现（review findings）的能力。该能力仅表达"疑似"技术模式，不输出确定性技术诊断，与 `rule-based-diagnostics` 的确定性诊断能力边界清晰分离，供教练复核而非自动结论。
